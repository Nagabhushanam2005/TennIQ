...
if [[ "$ACTION" == "setup" ]]; then
    ...
    $PYTHON_EXEC -m inference.inference_main --config "$CONFIG_FILE" --input "$FRAMES_DIR" --mode images --model-name TrackNetV4_TypeA --weights yolo_train/ball_detection/yolo12n_ball_fine_tune3/weights/best.pt --player-model "yolo_train/player_detection/yolo11n_player_finetune4/weights/best.pt" --bounce-model "models/ctb_regr_bounce.cbm"
fi